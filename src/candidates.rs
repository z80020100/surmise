...
mod tests {
    ...
    /// A throwaway directory tree. `tempfile` does this too. A short guard
    /// here keeps the dependency list empty instead.
    ///
    /// An entry that ends in `*` is made as a file rather than a directory.
